...
function downloadPlan() {
  downloadAsPdf(buildPlanHtml(), `debt-free-plan-${todayISO()}`);
}
...
// for the month currently shown in the calendar. Built live at click time.
function downloadMonthlyPackage() {
  ...
  const mStart = new Date(view.getFullYear(), view.getMonth(), 1);
  const mEnd = new Date(view.getFullYear(), view.getMonth() + 1, 0);

  const paydays = projectPaydays(mStart, mEnd);
  const bills = billOccurrences(mStart, mEnd);
  const monthTx = transactions.filter((t) => t.date.slice(0, 7) === ym).sort((a, b) => (a.date < b.date ? -1 : 1));

  // Calendar grid as an HTML table.
  const cells = [];
  for (let i = 0; i < mStart.getDay(); i++) cells.push('<td></td>');
  ...
  const cal = `<table class="cal"><tr>${['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((h) => `<th>${h}</th>`).join('')}</tr>${grid}</table>`;

  // Line items.
  const incomeRows = paydays.map((p) => `<tr><td>${p.date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</td><td>${escapeHtml(shortName(p.name) || p.name)}</td><td class="r in">${money(p.amount)}</td></tr>`).join('');
...
<table><tr><th>Date</th><th>What</th><th class="r">Cost</th></tr>${monthAppts.map((e) => `<tr><td>${e.date}</td><td>${escapeHtml(e.title)}</td><td class="r out">${e.cost ? money(e.cost) : '—'}</td></tr>`).join('')}</table>` : '';
  ...
  const byCat = {};
  for (const t of monthTx) { if (t.amount < 0 && isFlowCat(catOf(t))) byCat[catOf(t)] = (byCat[catOf(t)] || 0) + Math.abs(t.amount); }
...
<h2>📅 Calendar (💵 income · bills on due dates)</h2>
${cal}

<h2>💵 Income this month</h2>
<table><tr><th>Date</th><th>Source</th><th class="r">Amount</th></tr>${incomeRows}<tr class="tot"><td colspan="2">Total in</td><td class="r in">${money(totalIn)}</td></tr></table>
